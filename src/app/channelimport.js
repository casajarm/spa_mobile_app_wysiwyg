...
const StartSelector = Parse.Object.extend("StartSelector");
...
var groupSelected = Parse.Object.extend("Group"); // current group to be displayed in emulator (phone display window)
...
function displayStyle() {
	...
	document.getElementById("backgroundColor").jscolor.importColor();
	...
	document.getElementById("navColor").jscolor.importColor();

	$(".activeNavIcon").css("color", styles[0].get("tabBarSelected"));
	$("#activeNav").val(styles[0].get("tabBarSelected").substring(1));
	document.getElementById("activeNav").jscolor.importColor();
	...
	document.getElementById("textColor").jscolor.importColor();
	...
	document.getElementById("catTextColor").jscolor.importColor();
}

//cancel style edits

$("#editorThemeCancel").click(function() {
...
async function saveNewGroups() {
	return new Promise(async (resolve, reject) => {
		...
		Parse.Object.saveAll(newGroups).then(
			groups => {
				for (var i = 0; i < groups.length; i++) {
			...
			error => {
				reject();
				alert(
					"Failed to create new objects, with error code: " + error.message
				);
			}
		);
	});
...
async function cloneLikemojis() {
	...
	return await Parse.Object.saveAll(newLikemojisArray);
}
...
function setNewGroupOrder() {
	...
	Parse.Object.saveAll(groups).then(
		async groupsSaved => {
...
async function userLogin() {
	...
	const user = await Parse.User.logIn(user_email, user_password);
	...
	Parse.User.logOut().then(() => {
		...
		if (currentUser) {
			...
			window.location.href =
				"file:///Users/alexbarrett/Desktop/likemoji%20website%20and%20builder/likemoji%20" +
